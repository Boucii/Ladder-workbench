...
when(pt3==="b011".U && pt5==="b01000".U && pt6==="b11".U){    // sd     
  ...
  regs.io.raddr1:=rs1
  regs.io.raddr2:=rs2
  src1:=regs.io.rdata1;src2:=regs.rdata2
  io.Men:=1.U
  io.Mwout:=1.U
  io.Maddr:=src1+dest
  io.Mlen:=8.U
  ...
  src1:=regs.io.rdata1
  src2:=regs.io.rdata2
  regs.io.wdata:=Mux(src1.S<src2.S,0.U,1.U)
  regs.io.waddr:=dest
  regs.io.wen:=1.U
  ...
  src1:=regs.io.rdata1
  src2:=regs.io.rdata2
  regs.io.wdata:=src1.S>>src2(5,0).U//????
  regs.io.waddr:=dest
  regs.io.wen:=1.U
